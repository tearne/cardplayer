...
- 2026-05-06: Superseded by `audio-library-rollback`. Foxen FLAC was the bursty decoder this whole change was wrestling — it goes away with the rollback to ESP8266Audio's libFLAC, which shipped clean FLAC playback at ring depth 3 pre-migration. Speaker-side DMA queue tuning (`1024 × 8`) and channel-pinned `playRaw()` from this change carry over to the new `audio_output_m5.h`. Ring depth back to 3 slots reclaims the 9 KB this change spent. The on-device verification step is closed unticked — the decoder it would have tested is no longer present. Final shipped version of *this* change: `0.13.25`. Archived alongside the rollback.

## Conclusion

Superseded by `audio-library-rollback` on 2026-05-06. The deeper-ring + speaker-DMA + channel-pinning fixes worked around Foxen FLAC's burstiness; rolling back to ESP8266Audio's libFLAC removed the burstiness itself. Useful pieces (speaker-side `1024 × 8` DMA queue, channel-pinned `playRaw()`) carry over; the ring-depth bump reverts.
