...
- **Apply Tier 1 trims.** `SPEC_COLS` 480 → 256, `WV_COLS` 480 → 256, `RING_COUNT` 3 → 2, audio task stack 8 → 6 KB, viz task stack 4 → 3 KB.
- **Smoke-test on device.** Play a track, watch heatmap + waveform render without glitches. Check `stk` readout for the audio task — should stay well under 100 %. Verify no new underruns (`u` readout) and no viz crashes / fragmentation issues.
- **Verdict.** Measure free-heap percentage after the trim and judge whether Tier 2 is needed.

**Done when**

Tier 1 applied, RAM reduction confirmed, no regressions. Tier 2 either entered as a follow-up or judged unnecessary.

## Log

- 0.21.60: Tier 1 trims applied — `SPEC_COLS` 480 → 256, `WV_COLS` 480 → 256, `RING_COUNT` 3 → 2, audio stack 8 → 6 KB, viz stack 4 → 3 KB. Heap used dropped 86 % → 82 % (~12.8 KB freed, matching the ~12.9 KB estimate). No regressions on-device.

## Conclusion

Tier 1 done. ~13 KB freed bringing free heap to ~58 KB (18 %). Comfortable headroom, no need to escalate to Tier 2 or 3. Those tiers remain documented above as future options if RAM pressure returns.
